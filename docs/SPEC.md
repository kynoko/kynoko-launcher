# Kynoko Applications - Specification
...
Kynoko Applications is the desktop entry point to the Kynoko web apps on
...
| Catalogue | The public, read-only list of apps, façades and accepted file types that Kynoko Applications follows. |
...
| Agent | The resident part of Kynoko Applications that owns the bridge while files are open. |
...
```
 double-click foo.docx
        |
        v
 OS association ──> kynoko-applications open "C:\...\foo.docx"
                          |
...
- The exact URL of the catalogue is to be decided with the platform (it goes
  ...
  Kynoko Applications' settings for the dev domains.
...
- A failure retries with an exponential backoff (5 min, 15 min, 1 h, capped by
  the 12 h period).
- A failure is shown **only inside Kynoko Applications**, as a quiet line with
...
When a new catalogue arrives, Kynoko Applications applies the difference:
...
Every shortcut runs **Kynoko Applications**, not the browser:
`kynoko-applications launch <appCode>[/<façadePath>]`. The browser is resolved
...
| macOS | a small launcher `.app` in `~/Applications/Kynoko/<App name>/` | launcher `.app` next to it | `.icns` |
...
- Launcher `.app` bundles are created locally, so they carry no quarantine
...
|---|---|---|
| Windows | `HKCU\Software\Classes`: a ProgID `Kynoko.<App>.<ext>` (icon, verb `open` -> `kynoko-applications open "%1"`), `.<ext>\OpenWithProgids`, plus `Capabilities` + `RegisteredApplications` so that Kynoko Applications appears in Settings > Default apps | **Cannot be set programmatically** (hashed `UserChoice`). Kynoko Applications opens the Default apps page on its own entry and explains. |
| macOS | Document types are **static**: the `Info.plist` of Kynoko Applications declares every Kynoko type with `LSHandlerRank = Alternate`. Enabling an extension = making it the default | `NSWorkspace.setDefaultApplication(at:toOpenContentType:)` (may show a system confirmation) |
| Linux | `MimeType=` of a `kynoko-applications-open.desktop`, a shared-mime-info XML for types the system lacks, `update-desktop-database` | `xdg-mime default` / `~/.config/mimeapps.list` |

On macOS, files arrive through Apple Events, not `argv` (Tauri:
`RunEvent::Opened`).

Consequence of the macOS static declaration: while Kynoko Applications is
...
Kynoko Applications could neither list nor remove, and which would duplicate
...
- Created by Kynoko Applications only, from the operating system (association,
...
> Kynoko Office passe par Kynoko Applications. Votre navigateur va demander si
...
**The page never probes the bridge** to detect whether Kynoko Applications is
...
- **File source abstraction**: every opened document knows where it came from
  ...
  | Source | Where | Save |
  |---|---|---|
  | `bridge` | Kynoko Applications, any desktop browser | `PUT` with `If-Match`, in place |
  ...
  | `picker` | `<input type=file>` everywhere else (Firefox/Safari without Kynoko Applications, iOS) | copy: save = download |
...
- **Install invitation on desktop**: the "Install" banner and menu entry offer
  to download Kynoko Applications for the detected OS, instead of the browser's
...
| macOS | `.dmg` with `Kynoko Applications.app` | `/Applications` or `~/Applications` |
...
registry keys and values, `.desktop` and MIME files, launcher bundles and
...
- **macOS**: there is no uninstaller; dragging the app to the Trash runs no
  code. The window offers **"Uninstall Kynoko Applications"**, which cleans up
  then moves the app to the Trash. If the app is trashed directly, LaunchServices
  forgets its document types by itself, but launcher bundles and changed
...
- **Linux**: package removal scripts run as root and must not touch users'
  ...
  the window's "Uninstall" and `kynoko-applications cleanup` do the complete
...
Kynoko Applications checks the latest GitHub release with the same policy as
...
4. **Chromium installed PWAs**: can Kynoko Applications launch an installed PWA
...
| 2 | Kynoko Applications v1: catalogue, browsers and profiles, shortcuts, associations, agent, inventory and cleanup, i18n. |
...
| Date | Decision |
|---|---|
| 2026-09-25 | Kynoko Applications is the single desktop entry point, Chromium included; `file_handlers` removed from web manifests. |
